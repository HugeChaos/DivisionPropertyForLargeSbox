...
import copy
import CS_CIPHER.CS_CIPHER_model
import time
import os
...
if __name__ == "__main__":
    ...
    cd["cipher_name"] = "CS_CIPHER"

    cd["cipher_size"] = 64

    cd["inequalities"] = CS_CIPHER.CS_CIPHER_specify.inequalities_EDPPT1

    folder = cd["cipher_name"] + "_integral_distinguish_edppt1"
    ...
    for weight in range(6, 8):
        ...
        for b in get_b_search_space(weight):
            ...
            balance_position = ["?" for bpi in range(0, 64)]
            distinguish_find1 = False
            for e in e_search_space:
                cd["e"] = copy.deepcopy(e)
                CS_CIPHER.CS_CIPHER_model.model(cd)
                flag = CS_CIPHER.CS_CIPHER_model.model_solve(cd["solve_file"])
                if flag:
                    bll = e_dict[str(e)]
            ...
            if distinguish_find1:
                ...
                rf.write("*" * 20)
                rf.write("{}th  3round integral distinguish (weight = {}) based on division property"
